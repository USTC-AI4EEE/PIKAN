...
if __name__ == '__main__':
    ...
    for model in ['Transformer']: # 'MLP','CNN','KAN','KAN_small','Transformer'
        ...
        writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model}/{model}_TJU_results.xlsx')
        tju_gap = 0.07
        results = Results(root,gap=tju_gap)
    ...
    for model in ['Transformer']: # 'MLP','CNN','KAN','KAN_small','Transformer'
        ...
        for n in [1,2]:
            root = f'results_small-sample/{model}/TJU results (small sample {n})/'
            writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model}/{model}_TJU_results_small_sample_{n}.xlsx')  
            tju_gap = 0.07
            results = Results(root,gap=tju_gap)
            for batch in [2]:
                df_battery_mean = results.get_battery_average(train_batch=batch,test_batch=batch)
