...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model_name}/{model_name}_TJU_results.xlsx')
    ...
    for n in [1,2]:
        ...
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model_name}/{model_name}_TJU_results_small_sample_{n}.xlsx')
        results = Results(root, gap=0.07)
        for batch in [2]:
            df_battery_mean = results.get_battery_average(train_batch=batch,test_batch=batch)
            # df_experiment_mean = results.get_experiments_mean(test_batch=batch,train_batch=batch)
