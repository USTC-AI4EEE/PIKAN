...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model_name}/{model_name}_MIT_results.xlsx')
    ...
    for n in [1,2]:
        ...
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model_name}/{model_name}_MIT_results_small_sample_{n}.xlsx')
        results = Results(root)
        df_mean1 = results.get_battery_average()
        # df_mean2 = results.get_experiment_average()
        df_mean1.to_excel(writer, sheet_name='battery_mean_0', index=False)
