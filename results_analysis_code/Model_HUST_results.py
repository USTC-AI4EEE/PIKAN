...
if __name__ == '__main__':
    ...
    add_name = '_alpha0.01_beta0'

    # HUST soh-estimation
    root = f'results_soh-estimation/{model_name}{add_name}/HUST results/'
    save_folder = f'results_soh-estimation/processed_results/{model_name}{add_name}/'
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model_name}{add_name}/{model_name}{add_name}_HUST_results.xlsx')
    ...
    #     if not os.path.exists(save_folder):
    #         os.makedirs(save_folder)
    #     writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model_name}/{model_name}_HUST_results_small_sample_{n}.xlsx')
    #     results = Results(root)
    #     df_battery_mean = results.get_battery_mean()
    #     # df_experiment_mean = results.get_experiments_mean()
    #     df_battery_mean.to_excel(writer, sheet_name='battery_mean_0', index=False)
