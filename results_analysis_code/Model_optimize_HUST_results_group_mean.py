...
class Results:
    ...
    def get_group_mean(self,total_group=49):
        s_df_mean_values = []
        for group in range(1, 1+total_group):
            s = 'group' + str(group)
            df_mean_values = []
            for e in range(1, 11):
                res = results.get_test_results(e=e,s=s)
                df = pd.DataFrame(res)
                df = df[['MAE', 'MAPE', 'RMSE', 'R2']]
                df_i_mean = df.mean(axis=0)
                df_mean_values.append(df_i_mean.values)
...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model_name}/{model_name}_HUST_results_group_mean.xlsx')
    results = Results(root)
    df_group_mean = results.get_group_mean(total_group=49)
    ...
    for n in [1,2,3,4]:
        ...
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model_name}/{model_name}_HUST_results_small_sample_{n}_group_mean.xlsx')
        results = Results(root)
        df_group_mean = results.get_group_mean(total_group=49)
        df_group_mean.to_excel(writer, sheet_name='group_mean_0', index=False)
        writer._save()
