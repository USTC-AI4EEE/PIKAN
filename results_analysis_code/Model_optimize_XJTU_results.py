...
class Results:
    ...
    def _update_experiments(self,train_batch=0,test_batch=1,experiment=1):
        subfolder = f'{train_batch}-{test_batch}/Experiment{experiment}/best_results'
...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model_name}/{model_name}_XJTU_results_best.xlsx')
    ...
    for n in [1,2,3,4]:
        ...
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model_name}/{model_name}_XJTU_results_small_sample_{n}_best.xlsx')
        results = Results(root)
        for batch in [0]:
            df_battery_mean = results.get_battery_mean(train_batch=batch,test_batch=batch,total_experiment=10)
            df_battery_mean.to_excel(writer,sheet_name='battery_mean_{}'.format(batch),index=False)
