...
#     checkpoint = torch.load(os.path.join(self.args.save_folder,'model.pth'))
#     self.solution_u.load_state_dict(checkpoint['solution_u'])
#     self.dynamical_F.load_state_dict(checkpoint['dynamical_F'])
#     self.solution_u.encoder.auto_symbolic()
#     self.solution_u.predictor.auto_symbolic()
#     self.dynamical_F.dynamical_F.auto_symbolic()
#     ex_round(self.solution_u.encoder.symbolic_formula()[0][0],2)
#     ex_round(self.solution_u.predictor.symbolic_formula()[0][0],2)
#     ex_round(self.dynamical_F.dynamical_F.symbolic_formula()[0][0],2)
    
if __name__ == '__main__':

    model_name = 'PIKAN_opt'
    add_name = '_alpha0.1_beta0'
    # XJTU soh-estimation
    root = f'results_soh-estimation/{model_name}{add_name}/XJTU results/'
    save_folder = f'results_soh-estimation/processed_results/{model_name}{add_name}/'
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model_name}{add_name}/{model_name}{add_name}_XJTU_results.xlsx')
    results = Results(root)
    for batch in range(1):
        df_battery_mean = results.get_battery_mean(train_batch=batch,test_batch=batch,total_experiment=1)
    ...
    #     if not os.path.exists(save_folder):
    #         os.makedirs(save_folder)
    #     writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model_name}/{model_name}_XJTU_results_small_sample_{n}.xlsx')
    #     results = Results(root)
