...
class Results:
    ...
    def get_battery_average(self,train_batch=0,test_batch=0,subfolder='best_results'):
        df_mean_values = []
        for i in range(1,11):
            res = self.get_test_results(train_batch,test_batch,i,s=subfolder)
    ...
    def get_group_average(self,train_batch=0,test_batch=0,total_group=49):
        s_df_mean_values = []
        for group in range(1,1+total_group):   
            s = 'group' + str(group) 
            df_mean_values = []
            for i in range(1,11):
                res = self.get_test_results(train_batch,test_batch,i,s)
                df_i = pd.DataFrame(res)
                df_i = df_i[['MAE', 'MAPE', 'RMSE', 'R2']]
                df_mean_values.append(df_i.mean(axis=0).values)
            df_mean_values = np.array(df_mean_values)
...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model_name}/{model_name}_TJU_results_group_mean.xlsx')
