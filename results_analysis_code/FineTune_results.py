...
if __name__ == '__main__':
    ...
    if not os.path.exists(f'results_fine-tuning/processed_results/{model_name}/{finetune_method}'):
        os.makedirs(f'results_fine-tuning/processed_results/{model_name}/{finetune_method}')

    source = 'HUST'
    target = 'MIT'

    com_root = f'results_fine-tuning/{model_name}/{finetune_method}/{source}-{target}/'
    writer = pd.ExcelWriter(f'results_fine-tuning/processed_results/{model_name}/{finetune_method}/FineTune-{source}-{target}.xlsx')
    xjtu_gap = 0.05
    tju_gap = 0.07
    mit_gap = 0.05
    hust_gap = 0.1
    results = Results(com_root,gap=mit_gap)
    batch = 0
