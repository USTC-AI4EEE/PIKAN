...
if __name__ == '__main__':
    ...
    for model in ['Transformer']: # 'MLP','CNN','KAN','KAN_small','Transformer','KAN_medium'
        ...
        writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model}/{model}_XJTU_results.xlsx')    
        xjtu_gap = 0.05
    ...
    for model in ['Transformer']: # 'MLP','CNN','KAN','KAN_small','Transformer','KAN_medium'
        ...
        for n in [1,2,3,4]:
            root = f'results_small-sample/{model}/XJTU results (small sample {n})/'
            writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model}/{model}_XJTU_results_small_sample_{n}.xlsx')   
            xjtu_gap = 0.05
            results = Results(root,gap=xjtu_gap)
