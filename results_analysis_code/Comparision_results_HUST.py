...
if __name__ == '__main__':  
    ...
    for model in ['Transformer']: # 'MLP','CNN','KAN','BiLSTMTransformer','KAN_small','LSTM','TCN','Transformer'
        ...
        writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model}/{model}_HUST_results.xlsx')
    ...
    for model in ['Transformer']: # 'MLP','CNN','KAN','BiLSTMTransformer','KAN_small','LSTM','TCN','Transformer'
        ...
        for n in [1,2,3,4]:
            root = f'results_small-sample/{model}/HUST results (small sample {n})/' 
            writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model}/{model}_HUST_results_small_sample_{n}.xlsx')   
            results = Results(root)
            df_battery_mean = results.get_battery_mean()
            # df_experiment_mean = results.get_experiments_mean()
            df_battery_mean.to_excel(writer, sheet_name='battery_mean_0', index=False)
