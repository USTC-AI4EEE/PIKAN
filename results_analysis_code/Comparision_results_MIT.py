...
if __name__ == '__main__':
    ...
    for model in ['Transformer']: # 'MLP','CNN','KAN','KAN_small','Transformer'
        ...
        writer = pd.ExcelWriter(f'results_soh-estimation/processed_results/{model}/{model}_MIT_results.xlsx')
    ...
    for model in ['Transformer']: # 'MLP','CNN','KAN','KAN_small','Transformer'
        ...
        for n in [1,2]:
            root = f'results_small-sample/{model}/MIT results (small sample {n})/'
            writer = pd.ExcelWriter(f'results_small-sample/processed_results/{model}/{model}_MIT_results_small_sample_{n}.xlsx')   
            results = Results(root)
            df_mean1 = results.get_battery_average()
            df_mean1.to_excel(writer, sheet_name='battery_mean_0', index=False)
            # df_mean2 = results.get_experiment_average()
