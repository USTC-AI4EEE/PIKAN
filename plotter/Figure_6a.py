...
for data in ['XJTU','TJU','MIT','HUST']:
    ...
    for batch in batches:
        ############################
        df_list = []
        for model in ['VerhulstPIKAN', 'PIKAN', 'VerhulstPINN', 'PINN']:
        ...
        #                linewidth=1,  # 调整包络线粗细
        #                ax=ax)
        sns.boxplot(x='metric',y='error',hue='model',data=df,
                    dodge=True,
                    saturation=1,
                    palette=colors,
                    linewidth=1,
...
legend_labels = ['VerhulstPIKAN', 'PIKAN', 'VerhulstPINN', 'PINN']
axs[3, 2].legend(
    handles=boxs,
...
plt.tight_layout()
plt.savefig('../Figures/soh_estimation_boxplot_error.svg',format='svg')
plt.show()
