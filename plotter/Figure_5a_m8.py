...
def plot():
    ...
    for i in train_battery_num:
        ...
        df4['model'] = ['MLP'] * 10
        df4['train num'] = [i] * 10
        df5 = pd.read_excel(f'results_small-sample/processed_results/CNN/CNN_{data}_results_small_sample_{i}.xlsx',
                            engine='openpyxl',
                            nrows=10,
                            sheet_name=f'battery_mean_{batch}')
        df5['model'] = ['CNN'] * 10
        df5['train num'] = [i] * 10
        df6 = pd.read_excel(f'results_small-sample/processed_results/LSTM/LSTM_{data}_results_small_sample_{i}.xlsx',
                            engine='openpyxl',
                            nrows=10,
                            sheet_name=f'battery_mean_{batch}')
        df6['model'] = ['LSTM'] * 10
        df6['train num'] = [i] * 10
        df7 = pd.read_excel(f'results_small-sample/processed_results/TCN/TCN_{data}_results_small_sample_{i}.xlsx',
                            engine='openpyxl',
                            nrows=10,
                            sheet_name=f'battery_mean_{batch}')
        df7['model'] = ['TCN'] * 10
        df7['train num'] = [i] * 10
        df8 = pd.read_excel(f'results_small-sample/processed_results/Transformer/Transformer_{data}_results_small_sample_{i}.xlsx',
                            engine='openpyxl',
                            nrows=10,
                            sheet_name=f'battery_mean_{batch}')
        df8['model'] = ['TRF'] * 10
    ...
    merge_df_keys = ['PIKAN', 'KAN', 'PINN', 'MLP', 'CNN', 'LSTM', 'TCN', 'TRF']
    ...
    plt.tight_layout()
    plt.savefig(f'Figures/small_sample_{data}_batch_1_m8.svg',format='svg')
    plt.show()
