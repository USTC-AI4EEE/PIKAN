...
plt.style.use('own_style_4.mplstyle')
plt.rc('font', family='Times New Roman')

fig, axs = plt.subplots(4,3,figsize=(15,12.5),dpi=300)
# colors = ['#b8dff2','#abeadb',"#ffaf7a",'#ffb2b4']
colors = ["#f8a0a3","#fcba8f","#73e4c2","#93d9fc","#ddb3f8"]
count = 0
for data in ['XJTU','TJU','MIT','HUST']:
    ...
    for batch in batches:
        ############################
        df_list = []
        for model in ['PIKAN', 'KAN', 'PINN', 'MLP', 'BiLSTMTransformer']:
            if model == 'MLP' or model == 'KAN' or model == 'BiLSTMTransformer':
        ...
        merge_df_keys = ['PIKAN', 'KAN', 'PINN', 'MLP','BiLSTMTransformer']
        ...
        for i, metric in enumerate(['MAE', 'MAPE', 'RMSE']):
            for model in ['PIKAN', 'KAN', 'PINN', 'MLP','BiLSTMTransformer']:
                ...
                offset = 0.225
                x_pos = i + (model == 'BiLSTMTransformer') * 1.4* offset + (model == 'MLP') * 0.7* offset + (model == 'PINN') * 0* offset - (model == 'KAN') * 0.7* offset - (model == 'PIKAN') * 1.4* offset
                ...
                ax.plot([x_pos - 0.06, x_pos + 0.06], [model_mean, model_mean], color='red', linestyle='-', linewidth=1.5)
...
legend_labels = ['PIKAN', 'KAN', 'PINN', 'MLP', 'BLT', 'Mean', 'Mean $\pm$ Std']
axs[3, 2].legend(
    handles=boxs,
...
plt.tight_layout()
plt.savefig('../Figures/soh_estimation_violin_error_m5_v2.svg',format='svg')
plt.show()
