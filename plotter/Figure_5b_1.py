...
for i in train_battery_num:
    ...
    df2['model'] = ['KAN'] * 10
    df2['train num'] = [i] * 10
    df3 = pd.read_excel(f'../results_author/processed results (small sample)/PINN-{data}_results (small sample {i}).xlsx',
                        engine='openpyxl',
                        nrows=10,
                        sheet_name=f'battery_mean_{batch}')
    df3['model'] = ['PINN'] * 10
    df3['train num'] = [i] * 10
    df4 = pd.read_excel(f'../results_author/processed results (small sample)/MLP-{data}-results (small sample {i}).xlsx',
                        engine='openpyxl',
                        nrows=10,
                        sheet_name=f'battery_mean_{batch}')
    df4['model'] = ['MLP'] * 10
...
plt.tight_layout()
plt.savefig(f'../Figures/small_sample_{data}_box.svg',format='svg')
plt.show()