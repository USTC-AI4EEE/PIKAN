...
def plot():
    fig, axs = plt.subplots(4,3,figsize=(20,15),dpi=300)
    # colors = ['#b8dff2','#abeadb',"#ffaf7a",'#ffb2b4']
    colors = ["#f8a0a3","#fcba8f","#73e4c2","#93d9fc","#ddb3f8","#b8dff2","#abeadb","#ffaf7a","#ffb2b4"]
    count = 0
    for data in ['XJTU','TJU','MIT','HUST']:
        ...
        for batch in batches:
            ############################
            df_list = []
            for model in ['PIKAN', 'KAN', 'PINN', 'MLP', 'BiLSTMTransformer','CNN','LSTM','TCN','Transformer']:
                if model == 'MLP' or model == 'KAN' or model == 'BiLSTMTransformer' or model == 'CNN' or model == 'LSTM'or model == 'TCN' or model == 'Transformer':
            ...
            merge_df_keys = ['PIKAN', 'KAN', 'PINN', 'MLP','BiLSTMTransformer','CNN','LSTM','TCN','Transformer']
            ...
            for i, metric in enumerate(['RMSE']):
                for model in ['PIKAN', 'KAN', 'PINN', 'MLP','BiLSTMTransformer','CNN','LSTM','TCN','Transformer']:
                    ...
                    offset = 0.2
                    x_pos = i - (model == 'PIKAN') * 2.8* offset \
                        - (model == 'KAN') * 2.1* offset \
                        - (model == 'PINN') * 1.4* offset \
                        - (model == 'MLP') * 0.7* offset \
                        + (model == 'BiLSTMTransformer') * 0* offset \
                        + (model == 'CNN') * 0.7* offset \
                        + (model == 'LSTM') * 1.4* offset \
                        + (model == 'TCN') * 2.1* offset \
                        + (model == 'Transformer') * 2.8* offset                 # 绘制标准差线 (draw the standard deviation line)
                    ax.plot([x_pos, x_pos], [model_mean - model_std, model_mean + model_std], color='black', linestyle='-',
                            linewidth=1.5)
                    # 绘制均值线 (draw the mean line)
                    ax.plot([x_pos - 0.01, x_pos + 0.01], [model_mean, model_mean], color='red', linestyle='-', linewidth=1.5)
    ...
    legend_labels = ['PIKAN', 'KAN', 'PINN', 'MLP', 'BLT', 'CNN','LSTM','TCN','TRF','Mean', 'Mean $\pm$ Std']
    axs[3, 2].legend(
        handles=boxs,
        labels=legend_labels,
        loc=[0.15, -0.12],   # 两列：[-0.05, 0.1]
    ...
    plt.tight_layout()
    plt.savefig('Figures/soh_estimation_violin_error_m9.svg',format='svg')
    plt.show()
