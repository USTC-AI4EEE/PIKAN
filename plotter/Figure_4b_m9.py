...
def plot():
    ...
    colors = [palette2[7],"#f8a0a3","#fcba8f","#f8f3a7","#73e4c2",palette1[0],"#93d9fc",palette1[2],"#ddb3f8"]
    # # 调整亮度（降低亮度值使颜色更深）
    # colors = [mcolors.hsv_to_rgb((h, s, v * 0.95)) for h, s, v in mcolors.rgb_to_hsv([mcolors.to_rgb(c) for c in set1_colors])]
    ...
    for idx, (data, batch) in enumerate(adjusted_datasets):
        ...
        # 设置x轴刻度标签，字体大小增加2
        ax.set_xticklabels(['PIKAN', 'KAN', 'PINN', 'MLP', 'BLT', 'CNN', 'LSTM', 'TCN', 'TRF'])
        ax.set_xlabel(None)
        
        # 旋转x轴标签以便更好地显示，字体大小增加2
        plt.setp(ax.get_xticklabels(), rotation=45, ha='right', fontsize=16)  # 从12增加到14
    ...
    plt.savefig('Figures/soh_estimation_violin_error_m9.svg', format='svg', bbox_inches='tight')
    plt.show()
