...
def plot():
    ...
    colors = [palette2[7],"#f8a0a3","#fcba8f","#f8f3a7","#73e4c2",palette1[0],"#93d9fc","#ddb3f8"]
    # model_order = ['PIKAN', 'KAN', 'PINN', 'LSTM','MLP', 'CNN','TCN','Transformer']
    # # 调整亮度（降低亮度值使颜色更深）
    ...
    for idx, (data, batch) in enumerate(adjusted_datasets):
        ############################
        df_list = []
        for model in ['PIKAN', 'KAN', 'PINN', 'MLP', 'CNN','LSTM','TCN','Transformer']:
        ...
        merge_df_keys = ['PIKAN', 'KAN', 'PINN', 'MLP', 'CNN','LSTM','TCN','Transformer']
        ...
        # 设置x轴刻度标签，字体大小增加2
        ax.set_xticklabels(['PIKAN', 'KAN', 'PINN', 'MLP', 'CNN', 'LSTM', 'TCN', 'TRF'])
        ax.set_xlabel(None)
    ...
    legend_labels = ['PIKAN', 'KAN', 'PINN', 'MLP', 'CNN', 'LSTM', 'TCN', 'TRF', 'Mean', 'Mean $\pm$ Std']
    ...
    plt.savefig('Figures/soh_estimation_violin_error_m8.svg', format='svg', bbox_inches='tight')
    plt.show()
