...
class ParameterSensitivityAnalysis:
    ...
    def save_figure(self, fig, filename='MIT_PSA.svg'):
        """保存图片"""
        # 确保保存目录存在
    ...
    def run(self, filename='MIT_PSA.svg'):
        """执行完整的分析和绘图流程"""
        try:
            # 加载数据
            df = self.load_data()
...
if __name__ == '__main__':
    # 设置表格文件路径
    file_path = 'results_soh-estimation/processed_results/PIKAN_opt/PIKAN_opt_MIT_results_group_mean.xlsx'
    
    # 创建分析器实例
    analyzer = ParameterSensitivityAnalysis(file_path)
    
    # 执行分析和绘图
    analyzer.run(filename='MIT_PSA.svg')
