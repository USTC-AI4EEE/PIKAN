...
class ParameterSensitivityAnalysis:
    ...
    def load_data(self):
        ...
        if 'group' not in df.columns or 'RMSE' not in df.columns:
            raise ValueError("表格中缺少必要的列: 'group' 或 'RMSE'")
        
        # 检查数据是否包含49个group
        if len(df) != 49:
            print(f"警告: 数据包含 {len(df)} 行，而不是期望的49行")
    ...
    def save_figure(self, fig, filename='HUST_PSA.svg'):
        """保存图片"""
        # 确保保存目录存在
    ...
    def run(self, filename='HUST_PSA.svg'):
        """执行完整的分析和绘图流程"""
        try:
            # 加载数据
            df = self.load_data()
...
if __name__ == '__main__':
    # 设置表格文件路径
    # file_path = 'results_soh-estimation/processed_results/PIKAN_opt/PIKAN_opt_HUST_results_group_mean.xlsx'
    file_path = 'results_small-sample/processed_results/PIKAN_opt/PIKAN_opt_HUST_results_small_sample_1_group_mean.xlsx'
    # 创建分析器实例
    analyzer = ParameterSensitivityAnalysis(file_path)
    
    # 执行分析和绘图
    analyzer.run(filename='HUST_PSA_small_sample_1.svg')
