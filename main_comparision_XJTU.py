...
def get_args():
    ...
    parser.add_argument('--model',type=str,default='MLP',choices=['MLP','CNN','KAN','BiLSTMTransformer','KAN_small','LSTM','TCN','Transformer','KAN_medium'])
...
def main_2():
    ...
    setattr(args,'dataset','XJTU') # select dataset: 'XJTU'
    for i in [0]:
...
def main():
    args = get_args()
    xjtu_batch_names = ['2C','3C','R2.5','R3','RW','satellite']
    setattr(args,'model','KAN_medium') # select model: MLP or CNN or KAN or BiLSTMTransformer
    setattr(args,'dataset','XJTU') # select dataset: 'XJTU'
    for i in range(6): # len(xjtu_batch_names)
        setattr(args,'xjtu_batch',xjtu_batch_names[i])
        for e in range(10):
            set_seed(e) 
            setattr(args,'save_folder',os.path.join(f'results_soh-estimation/{args.model}/',f'{args.dataset}-{args.model} results/{i}-{i}/Experiment{e+1}'))
...
def small_sample():
    ...
    for n in [1,2,3,4]:
        for i in range(len(xjtu_batch_names)):
            ...
            for e in range(10):
                set_seed(e) 
                save_folder = f'results_small-sample/{args.model}/XJTU results (small sample {n})/'+f'{i}-{i}/Experiment{e+1}'
                if not os.path.exists(save_folder):
                    os.makedirs(save_folder)
                setattr(args, "save_folder", save_folder)
                model = load_model(args)
                data_loader = load_XJTU_data(args,small_sample=n)
                trainer = Trainer(model,data_loader['train'],data_loader['valid'],data_loader['test'],args)
                trainer.train()
