...
class Solution_u(nn.Module):
    def __init__(self):
        ...
        self.encoder = KAN([17,60,60,32])
        self.predictor = KAN([32,32,1])
...
class PIKAN(nn.Module): # 手动调整损失函数权重，0.7,0.2
    def __init__(self,args):
        ...
        self.dynamical_F = KAN([35,60,60,1]).to(device) # 用于计算动态方程的网络
    ...
    def train_one_epoch(self,epoch,dataloader):
        ...
        for iter,(x1,x2,y1,y2) in enumerate(dataloader):
            ...
            debug_info = "[train] epoch:{} iter:{} data loss:{:.6f}, " \
                         "PDE loss:{:.6f}, physics loss:{:.6f}, total loss:{:.6f}".format(epoch,iter+1,loss1,loss2,loss3,loss.item())
