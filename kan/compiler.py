...
# # 定义sigmoid的符号函数类（用于SymPy表达式）
# class Sigmoid(Function):
#     nargs = 1  # 仅接受1个参数

# # 定义ReLU的符号函数类（用于SymPy表达式）
# class ReLU(Function):
#     nargs = 1  # 仅接受1个参数
...
def expr2kan(input_variables, expr, grid=5, k=3, auto_save=False):
    ...
    def create_node(expr, parent=None, n_layer=None):
        ...
        #     Connection([1,0,1,0], expr.func, fun_name = expr.func, parent=subnode, child=new_node, power_exponent=power_exponent)
            ...
            # expr.func 是1D函数（如sigmoid、relu等）
            node = Node(expr, False, depth, scale, bias, parent=parent)
            expr_i, scale_i, bias_i = next_nontrivial_operation(expr.args[0])
            subnode = SubNode(expr_i, node.depth + 1, scale_i, bias_i, parent=node)
            # 创建子节点（函数输入参数）
            new_node = create_node(expr.args[0], parent=subnode, n_layer=n_layer)
            ...
            if expr.func == Pow:
                power_exponent = expr.args[1]
                # 处理Pow的fun_name（保持原有逻辑）
                if power_exponent == Rational(1, 2):
                    fun_name = 'x^0.5'
                elif power_exponent == -Rational(1, 2):
                    fun_name = '1/x^0.5'
                elif power_exponent == Rational(3, 2):
                    fun_name = 'x^1.5'
                else:
                    alpha = int(power_exponent)
                    fun_name = f'x^{alpha}' if alpha > 0 else f'1/x^{-alpha}'
            else:
                # 自定义函数：sigmoid和relu的fun_name设为小写
                fun_name = expr.func.__name__.lower()  # Sigmoid -> "sigmoid", ReLU -> "relu"
                power_exponent = None
            
            # 创建连接（关联符号函数和torch实现）
            Connection(
                affine=[1, 0, float(scale), float(bias)],
                fun=expr.func,  # SymPy函数类（用于符号表达式）
                fun_name=fun_name,  # 函数名（用于映射到torch实现）
                parent=subnode,
                child=new_node,
